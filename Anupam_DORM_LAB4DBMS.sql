...
select b.cus_gender, count(b.cus_gender) from (select p.cus_id, p.cus_gender, count(p.order_id) from
(select `order` .*, Customer.cus_gender, Customer.cus_name from customer inner join `order` on 
customer.cus_id=`order`.cus_id group by `order`.order_id having `order`.order_amount>=3000) 
as p group by p.cus_id) as b group by b.cus_gender;
...
select `order`.*, p.pro_name from (select product.*, supplier_pricing.pricing_id, supplier_pricing.sup_id, supplier_pricing.sup_price from
product inner join supplier_pricing on product.pro_id=supplier_pricing.pro_id) as p inner join `order`
on p.pricing_id=`order`.pricing_id where `order`.cus_id=2;


-- 5)Display the Supplier details who can supply more than one product.--

select s.* from supplier s where s.sup_id in
(select sp.sup_id from supplier_pricing sp group by sp.sup_id having count(sup_id)>2)
group by s.sup_id;

-- 6)Find the least expensive product from each category and print the table with category id, name, product name and price of the product

select c.cat_id, c.cat_name, t3.pro_name, min(t3.Min_Price) from 
(select p.*, t2.Min_price from (Select pro_id, min(sup_price) as Min_Price from supplier_pricing Group by pro_id)
as t2 inner join product p on t2.pro_id=p.pro_id) as t3 inner join category c 
on t3.cat_id=c.cat_id group by t3.cat_id;
